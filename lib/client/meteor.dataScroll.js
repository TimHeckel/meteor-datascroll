Meteor.DataScroll = function(_opts) {
	var _self = this, _sessionMultiplePages = '__hasMultiplePages__', _sessionButtons = "__sessionButtons__";
	...
	var _setPagination = function() {
    	...
    	console.log("total recoreds: ", opts.pagination.totalRecords, opts.pagination.resultsPerPage);
		...
		Template[opts.templates.rpp].events({
			"change select": function(e, template) {
				if (opts.isActive) {
					...
					_self.changeResultsPerPage(_rpp);
