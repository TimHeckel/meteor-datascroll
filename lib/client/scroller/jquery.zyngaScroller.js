...
(function ($) {
    ...
    var locals = {
        ...
        , resizeAndPositionContents: function(_self) {
            if (_self.data("zyngaScroller")) {
                var opts = _self.data("zyngaScroller").options
                    , _left = $(opts.es.content).css("transform").split(' ').length > 3 ? parseInt($(opts.es.content).css("transform").split(' ')[4].replace(',', '')) * -1 : 0;
    ...
    var methods = {
            ...
            return this.each(function () {
                ...
                if (!_self.data('zyngaScroller')) {
                    //window.setTimeout(function() {
                        ...
                        var dragCursor = ff ? "-moz-grab" : "url('openhand.cur')" + cursCoords + ", move";
                        _self.css({ cursor: dragCursor });
                        _self.mousedown(function (e) {
                            dragCursor = ff ? "-moz-grabbing" : "url('closedhand.cur')" + cursCoords + ", move";
                            _self.css({ cursor: dragCursor });
                        });
                        _self.mouseup(function (e) {
                            dragCursor = ff ? "-moz-grab" : "url('openhand.cur')" + cursCoords + ", move";
                            _self.css({ cursor: dragCursor });
                        });
        ...
        enableScroll: function () {
            return $(this).data("zyngaScroller").options.es.options.enabled = true;
        },
        disableScroll: function () {
            return $(this).data("zyngaScroller").options.es.options.enabled = false;
        },
        destroy: function () {
            return this.each(function () {
                var _self = $(this);
